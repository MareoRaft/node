...
sub deleteNodeNamed{
	my ($name) = @_;
	if( !nameExists($name,\@nodes) ){ return 0 }
...
sub connectedGraph{ #print "input is ",@_," and family is ",family(@_)," and lengthleft is ",length(@_)," and lengthright is ",length(family(@_)),"\n";
	my ($currentref) = @_;
	if( intersect($currentref,[nodeNames])==() ){ return }
...
sub viewData{ #print "\$#nodes is $#nodes\n";
	my ($name) = @_;
	unless( $name eq '' ){
		if( nameExists($name,\@nodes) ){
			print "\nNode ",RED $name,RESET " points to: "; display(obj($name)->connectorsRef); print "\n\n"; return 1
		}
		print "$input is not a node.\n"; return 1
	}
	if( $#nodes==-1 ){ print "There is no data currently.\n"; return 1 }
	print "\n";
	foreach( @nodes ){ print "Node ",RED $_->name,RESET " points to: "; display($_->connectorsRef); print "\n\n" }
	return 1;
}
...
sub changeGraphType(){
	...
	if( $graphtype eq 'graph' ){
		print "Successfully switched from ", BOLD "directed", RESET " graph mode to ", BOLD "graph", RESET " mode.  From now on, all connections you create will point both ways.  Would you like to convert all directed connections to regular connections?\n";
		my $i = promptInput(); if( $i =~ /^y/ ){ completeConnections(); print "Successfully completed connections.\n" }
	}
	elsif( $graphtype eq 'directed graph' ){
	  print "Successfully switched from ", BOLD "graph", RESET " mode to ", BOLD "directed", RESET " graph mode.  From now on, all connections you create will point one way.\n"
	}
}
