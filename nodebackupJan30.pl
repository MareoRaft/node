...
my %colorstrawberryonwhite = ( 'name' => "\x1b[38;5;162m", 'nameinput' => "\x1b[35m", 'command' => "\x1b[38;5;19m", 'commandinput' => "\x1b[38;5;26m" );
my %colorstrawberryonblack = ( 'name' => "\x1b[38;5;162m", 'nameinput' => "\x1b[35m", 'command' => "\x1b[38;5;27m", 'commandinput' => "\x1b[38;5;33m" );
my %colordefaultonblack = ( 'name' => "\x1b[38;5;142m", 'nameinput' => "\x1b[38;5;208m", 'command' => "\x1b[38;5;38m", 'commandinput' => "\x1b[38;5;32m" );
#defaultonwhite still needs tweaking!
my %colordefaultonwhite = ( 'name' => "\x1b[38;5;142m", 'nameinput' => "\x1b[38;5;208m", 'command' => "\x1b[38;5;38m", 'commandinput' => "\x1b[38;5;32m" );
my %colornaturalonwhite = ( 'name' => "\x1b[38;5;22m", 'nameinput' => "\x1b[38;5;24m", 'command' => "\x1b[38;5;238m", 'commandinput' => "\x1b[38;5;94m" );
my %colornaturalonblack = ( 'name' => "\x1b[38;5;28m", 'nameinput' => "\x1b[38;5;24m", 'command' => "\x1b[38;5;243m", 'commandinput' => "\x1b[38;5;94m" );
my %color = %colordefaultonblack;
...
sub readFile{
	...
	open( IN, "$inname.txt" ) or return 0;
		local $/ = undef;
		my $data = <IN>;
	close( IN );
	return trim($data)
...
sub saveSettings{
    ...
    my $color = Data::Dumper->Dump( [\%color], ['color'] ); $color =~ s/\$/%/g; $color =~ s/'/"/g; $color =~ s/\{/\(/g; $color =~ s/\}/\)/g; $color =~ s/\{/\(/g; $color =~ s/\[/\\x1b\[/g;
    print OUT $color, "\n";
	close( OUT );
}
...
sub color{
  ...
  print "Color scheme choices are $color{'command'}strawberry$resetC, $color{'command'}natural$resetC, and $color{'command'}default$resetC.  You can also $color{'command'}create your own$resetC.\n";
  my $i = promptInput();
  ...
  $i = promptInput();
  if( $i =~ /^[lw]/i ){ $background = 'onwhite' }
  else{ $background = 'onblack' }
  %color = %colors{$background}{$choice};
}
...
sub redoData{ #print "\$historynum is $historynum\n";
	...
	deleteAllNodes();
	++$historynum; loadDataFrom(".history$historynum") or die "failed load"; print "Successfully redid last change.\n";
	return 1;
}
...
#	elsif( $i =~ /^[fk]/i ){ findData($c) }
	elsif( $i =~ /^col/i ){ color($c); saveSettings() }
