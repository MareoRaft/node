...
my $dt = 1;
my @positions; my @previouspositions;

sub trim{ my $i = shift @_; $i =~ s/^\s+|\s+$//g; return $i }

sub input{ my $i = <STDIN>; return trim($i) }

sub promptInput{ print "$filename \$ "; print GREEN; my $input = input(); print RESET; return $input }
sub promptInputOrange{ print "$filename \$ "; print "\033[38;5;208m"; my $input = input(); print RESET; return $input }
...
sub addConnections{
	my $node = shift @_; #print "gonna connects $node to @_ END\n";
	if( @_ == () ){ @nodes = addElement($node,\@nodes) }
	foreach( @_ ){ addConnection($node,$_) }
	return 1;
}
...
sub addNodeNamed{
	my $node = shift @_; my $somethingnew=0;
	if( !isElement($node,\@nodes) ){ $somethingnew=1; @nodes = addElement($node,\@nodes) }
	print "What would you like ", RED, $node, RESET, " to be connected to? (Press ", CYAN, "RETURN", RESET, " after each entry, and when finished)\n";
	my @connectors = inputArray();
	unless( isSubset(\@connectors,\@$node) ){ $somethingnew=1; addConnections($node,@connectors) }
	if( $somethingnew==1 ){ return 1 }
	return 0
}
...
sub addNode{
	...
	if(addNodeNamed($node)){ print "Successfully added information.\n" and return 1 }
...
sub saveDataTo{
	my $outname = shift @_; $outname =~ s/\.txt$//i; my $out;
...
sub readFile{
	my $inname = shift @_; $inname =~ s/\.txt$//i;
